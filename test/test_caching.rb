...
class TestCaching < Sprockets::TestCase
  ...
  test 'unknown cache keys are ignored' do
    @env1.register_dependency_resolver 'foo-version' do |_env|
      1
    end
  ...
  test 'add/remove file to shadow vendor' do
    ...
    sandbox patched_jquery do
      File.utime(1_421_000_000, 1_422_000_000, File.dirname(patched_jquery))
      ...
      File.utime(1_421_000_020, 1_422_000_020, File.dirname(patched_jquery))
  ...
  test 'add/remove index file to shadow vendor' do
    ...
    sandbox File.dirname(patched_jquery), patched_jquery do
      File.utime(1_423_000_000, 1_423_000_000, File.dirname(File.dirname(patched_jquery)))
      ...
      File.unlink(patched_jquery)
      File.utime(1_423_000_020, 1_423_000_020, File.dirname(patched_jquery))
  ...
  test 'history cache is not polluted' do
    ...
    sandbox dependency_file do
      ...
      cache_entries = @cache.send(:find_caches).map do |file, _|
        ...
        result if result.is_a?(Array) && result.first.is_a?(Set)
      end.compact
