...
module SharedSassEmbeddedTestNoFunction
  ...
  test "aren't included globally" do
    ...
    assert_equal <<-EOS.chomp, sass.compile(filename, **options).css
div {
...
module SharedSassEmbeddedTestSprockets
  extend Sprockets::TestDefinition
...
module SharedSassEmbeddedTestCompressor
  extend Sprockets::TestDefinition
...
module SharedSassEmbeddedTestFunctions
  extend Sprockets::TestDefinition
...
@font-face {
  ...
  test "path function generates links" do
    asset = @env.find_asset('sass/paths.css')
    assert asset
