  ...
  module Loader
    ...
    def load_from_unloaded(unloaded)
      unless file?(unloaded.filename)
        fail FileNotFound, "could not find file: #{unloaded.filename}"
      end
      ...
      unless load_path
        ...
        fail FileOutsidePaths, "#{target} is no longer under a load path: #{paths.join(', ')}"
      end
      ...
      if type != file_type && !config[:transformers][file_type][type]
        fail ConversionError, "could not convert #{file_type.inspect} to #{type.inspect}"
      end
      ...
      if processors.any?
        source_uri, = resolve!(unloaded.filename, pipeline: :source)
        source_asset = load(source_uri)

        source_path = source_asset.digest_path

        result = call_processors(processors,             environment: self,
                                                         cache: cache,
                                                         uri: unloaded.uri,
