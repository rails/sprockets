  ...
  class DirectiveProcessor
    ...
    def compile_header_pattern(comments)
      re = comments.map do |c|
        ...
        when Array
          ...
          fail TypeError, "unknown comment type: #{c.class}"
        end
      end.join('|')
    ...
    def process_require_self_directive
      if @required.include?(@uri)
        fail ArgumentError, 'require_self can only be called once per source file'
      end
      @required << @uri
    end
    ...
    def expand_relative_dirname(directive, path)
      if @environment.relative_path?(path)
        ...
        if stat && stat.directory?
          ...
          fail ArgumentError, "#{directive} argument must be a directory"
        end
      else
        # The path must be relative and start with a `./`.
        fail ArgumentError, "#{directive} argument must be a relative path"
      end
    end
    ...
    def resolve(path, **kargs)
      ...
      if @environment.absolute_path?(path)
        fail FileOutsidePaths, "can't require absolute file: #{path}"
      end
